...
function PostDetail() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#0e1113] py-8 px-4">
      <button onClick={() => navigate(-1)} className="text-indigo-600 dark:text-indigo-400 flex items-center mb-6">
      ...
      <div className="max-w-4xl mx-auto bg-white dark:bg-[#1a1c1f] p-8 rounded-xl shadow">
        <header className="flex items-center mb-4">
          ...
          />
          <div className="ml-4">
            <h2 className="text-lg font-semibold text-gray-800 dark:text-white">{post.user.userName}</h2>
          ...
          {isOwner && (
            <div className="ml-auto relative" ref={dropdownRef}>
              <button onClick={() => setShowOptions(prev => !prev)} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-full">
                <Bars className="w-5 h-5 text-gray-600 dark:text-gray-300" />
              </button>
              {showOptions && (
                <div className="absolute right-0 mt-2 w-36 bg-white dark:bg-gray-800 rounded shadow z-50">
                  <button onClick={handleDeletePost} className="flex items-center px-4 py-2 text-sm text-red-600 hover:bg-gray-100 dark:hover:bg-gray-700 w-full">
        ...
        <h1 className="text-3xl font-bold text-indigo-600 dark:text-indigo-400 mb-4">{post.title}</h1>
        ...
        <div className="flex justify-between items-center border-t pt-6 dark:border-gray-700">
          ...
          <div className="flex gap-4">
            <button onClick={handleSavePost} className="flex items-center gap-1 text-sm text-gray-600 dark:text-white hover:text-indigo-600">
              {isSaved ? <UnSave className="w-5 h-5" /> : <Save className="w-5 h-5" />}
              {isSaved ? 'Unsave' : 'Save'}
            </button>
            <button onClick={handleShare} className="flex items-center gap-1 text-sm text-gray-600 dark:text-white hover:text-indigo-600">
